...
async function reserveSlotAtomic(conn, { slotKey }) {
  ...
  const capNum =
    typeof capRaw === "number" ? capRaw : parseInt(String(capRaw ?? ""), 10);
...
module.exports = {
  ...
  getAvailability: async () => {
    ...
    let fullyBooked = row.fully_booked_dates;
    try {
      if (typeof fullyBooked === "string")
        fullyBooked = JSON.parse(fullyBooked || "[]");
    } catch {
      fullyBooked = [];
    }
    const fully_booked_dates = Array.isArray(fullyBooked) ? fullyBooked : [];

    let bookedRaw = row.booked_slots;
    try {
      if (typeof bookedRaw === "string")
        bookedRaw = JSON.parse(bookedRaw || "[]");
    } catch {
      bookedRaw = {};
    }

    const booked_slots = normalizeBookedToMap(bookedRaw);

    const capRaw = row.max_capacity_per_slot;
    const capNum =
      typeof capRaw === "number" ? capRaw : parseInt(String(capRaw ?? ""), 10);
    const max_capacity_per_slot =
      Number.isFinite(capNum) && capNum > 0 ? capNum : 1;
